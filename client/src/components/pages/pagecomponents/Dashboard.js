...
function Dashboard(props) {
  ...
  function Profile() {
    return (
      <div className="profilecont" onMouseLeave={() => setOpenProfile(false)}>
        <AccountCircleIcon
          style={{ position: "relative", float: "center", fontSize: "400%" }}
        />
        <br />
        {userInfo.name}
        <br />
        ...
        <h5
          ...
          onClick={() => {
            window.confirm("Logout ?") && handleLogout()
          }}
        >
          Logout
        </h5>
  ...
  function JoinClass() {
    ...
    function handleSubmit() {
      JoinSubject({ userId: userInfo._id, subjectCode: subjectCode }).then(
        res => {
          window.alert(res)
          props.setupdate()
        }
    ...
    return (
      <div className="dashlink">
        <ul>
          ...
          <Link to={`/subject/${props.subjectId}`}>
            <li>Stream</li>
          </Link>
          <Link to={`/people/${props.subjectId}`}>
            <li>People</li>
          </Link>
        </ul>
      </div>
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="dashboard flex flex-row justify-between items-center lg:px-8 px-4 py-4">
        <img
          src="/logo-black.svg"
          alt="Edustellar Logo"
          width={200}
